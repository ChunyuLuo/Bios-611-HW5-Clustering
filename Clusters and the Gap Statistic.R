...
png(
  file.path(fig_dir, "Gap_Statistic_Simulation.png"),
  width = 1600,
  height = 1000
)
...
for (n in dimensions) {
  ...
  plot(
    subset_data$side_length,
    subset_data$estimated_k,
    type = "b",
    pch = 19,
    col = "blue",
    xlab = "Side Length (L)",
...
cat("Analysis of critical side lengths:\n\n")
...
for (n in dimensions) {
  ...
  if (nrow(underestimated) > 0) {
    ...
    cat(sprintf("  Gap statistic begins underestimating at L = %.1f\n", critical_L))
...
cat("\n=== SUMMARY ===\n")
cat("According to the simulation results, the Gap Statistic consistently begins\n")
cat("to reduce its estimate when L drops to approximately 2-3. This corresponds\n")
cat("to a critical ratio of L/sigma = 2-3, meaning cluster centers must be\n")
cat("separated by at least 2-3 times the within-cluster standard deviation\n")
cat("for reliable detection.\n")

cat("\nPlot saved to:", file.path(fig_dir, "Gap_Statistic_Simulation.png"), "\n")